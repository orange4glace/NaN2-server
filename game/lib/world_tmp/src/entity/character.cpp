...
namespace nan2 {
  ...
  void Character::FixedUpdate() {
    ...
    L_DEBUG << weapon_->magazine() << " " << weapon_->ammo() << " " << weapon_->cooldown() << " " << weapon_->reload_time() << " " << position_ << " " << dash_time_ << " " << dash_cooldown_ << " " << dash_dir_;
    snapshot_.SetWeaponMagazine(weapon_->magazine());  
    snapshot_.SetWeaponAmmo(30);
//    snapshot_.SetWeaponMagazine(weapon_->magazine());  
//    snapshot_.SetWeaponAmmo(weapon_->ammo());
  ...
  void Character::MoveTo(float x, float y) {
    L_DEBUG << "Move to " << x << " " << y << " " << player_->id();
  ...
  void Character::AddInput(PlayerInputPacket& plp) {
    L_DEBUG << "Input added " << player_->id();
